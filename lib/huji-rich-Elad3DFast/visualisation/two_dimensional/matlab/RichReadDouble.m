function [X,Pressure,Density,xVelocity,yVelocity,Points,time,Tracers,NumberOfPointsInCell]=RichReadDouble(filename,ShouldPlot,WhatToPlot,LogScale)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Matlab script to read RICH binary files in double format
%
...
%             cell
%   Example : [X,Pressure,Density,Points,xVelocity,yVelocity,time,Tracers,NumberOfPointsInCell]=RichReaddouble("output.bin",1,1,1)
%               Plots a log scaled density plot from the file "output.bin"
%             [X,Pressure,Density,Points,xVelocity,yVelocity,time,Tracers,NumberOfPointsInCell]=RichReaddouble("output.bin",0)
...
fid=fopen(filename);
N=fread(fid,1,'int');
data=fread(fid,N*2,'double');
X=reshape(data,2,N);
Pressure=fread(fid,N,'double');
Density=fread(fid,N,'double');
xVelocity=fread(fid,N,'double');
yVelocity=fread(fid,N,'double');
...
Tracers=fread(fid,TracerLength*TracerDim,'double');
