...
public:
    ...
    inline const string GetName() const { return ClmName; }

    inline const color_t GetColor(double aVal) const { return Clm[GetColorIndex(aVal)]; }

    const vector<unsigned char> GetColorTexture() const //float aColorMap
    {
        // TODO 255 is a magic num, make constexpr
        vector<unsigned char> texture;
         for(auto it = Clm.begin(); it != Clm.end(); it++){
