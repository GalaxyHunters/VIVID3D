...
class VolumetricViewer(object):
  """
  Utility Class for volumetric data previsualization. **IPython Required**
  ...
  def view(self):
    ...
    controls1 = widgets.VBox([title, widgets.HBox([label1, thresholds])], layout=widgets.Layout(display='flex', flex_flow='column', align_items='center'))
    controls2 = widgets.HBox([label2, play, slider, cm_dropdown], layout=widgets.Layout(display='flex', flex_flow='row', justify_content='center', align_items='center'))
  ...
  def render_animation(self, fps=25, min_threshold=None, max_threshold=None, cmap=None):
    ...
    animation_3d = animation.FuncAnimation(fig, animate, self.n_frames, interval=1000/fps, blit=False, cache_frame_data=False)
